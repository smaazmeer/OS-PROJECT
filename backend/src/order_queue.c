...
// Search the queue for a specific order and mark it as cancelled
void cancel_order_in_queue(OrderQueue *queue, int order_id) {
    ...
    for (int i = 0; i < count; i++) {
        int index = (queue->front + i) % QUEUE_SIZE;
        if (queue->orders[index].order_id == order_id && queue->orders[index].status == PENDING) {
            queue->orders[index].status = CANCELLED;
            // Note: Resources (cash/shares) should be unlocked by the API handler that called this
            break; 
        }
    }
